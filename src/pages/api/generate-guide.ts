...
export const POST: APIRoute = async ({ request }) => {
  ...
  const { data: consumed } = await supabase.rpc('consume_guide_credit', { p_user: selection.user_id });
  if (!consumed) return new Response('payment_required', { status: 402 });

  const refund = async () => { await supabase.rpc('add_guide_credits', { p_user: selection.user_id, p_amount: 1 }); };
